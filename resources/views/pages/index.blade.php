...
<div class="container mx-auto p-5">
        ...
        @foreach($posts as $post)
                <div class="border-b mb-5 pb-5 border-gray-200">
                    <img class="w-full" src="../../../storage/post_images/{{$post->post_image}}" alt="image">
                    <a href="/post/{{ $post->id }}" class="text-2xl font-bold mb-5">{{ $post->title }}</a>
                    <p class="text-grey-darker mb-6">{!!$post->details !!}
                   {{ str_limit($post->details, $limit = 150, $end = '...') }}
                    </p>
                    <p class="text-current mt-6"> on {{ date('M d Y', strtotime($post->created_at)) }} - {{ $post->user->name }}</p>
                </div>
            @endforeach
        </div>-->
    </div>
